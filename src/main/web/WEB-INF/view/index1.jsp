...
<body>
<form action="/form" method="get"><input type="submit" value="Form"></form>
<table width="100%" border="1">
    <tr>
        <td height="50">
            <form:form action="/user/add" method="post" modelAttribute="user" commandName="user">
                <form:input path="username" type="text" name="username" placeholder="Введите имя пользователя" size="30"/>
                <form:input path="password" type="password" name="password" placeholder="Введите пароль"/>

                <form:label path="roles">Role</form:label>
                <select name="roles[0].id">
                    <option value="NONE">Select Role</option>
                    <c:forEach items="${listRole}" var="role" varStatus="status">
                        <option value='<c:out value="${role.id}"/>'><c:out value="${role.role}"></c:out></option>
                    </c:forEach>
                </select>

                <%--<form:checkboxes path="roles" items="${listRole}"/>--%>
